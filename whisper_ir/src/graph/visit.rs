...
pub trait Visit<'graph> {
    fn visit_symbol(&mut self, ir_graph: &'graph IrTermGraph, symbol: SymbolIndex) {
    ...
    fn visit_ident(&mut self, _ir_graph: &'graph IrTermGraph, _ident: &Ident) {}
    fn visit_int32(&mut self, _ir_graph: &'graph IrTermGraph, _int: i32) {}
    fn visit_uint32(&mut self, _ir_graph: &'graph IrTermGraph, _int: u32) {}
    fn visit_float32(&mut self, _ir_graph: &'graph IrTermGraph, _int: f32) {}
...
pub fn visit_symbol<'graph, V>(v: &mut V, ir_graph: &'graph IrTermGraph, symbol: SymbolIndex)
where
    V: Visit<'graph> + ?Sized,
{
    let symbol = ir_graph.symbols().read()[symbol].clone();
    v.visit_ident(ir_graph, symbol.ident());
    v.visit_symbol(ir_graph, symbol.parent());
}
