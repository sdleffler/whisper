use crate::{graph::Blob, trans::CompoundKind, Name, SymbolIndex, SymbolTable, Var};
...
impl<'re, R: TermReader<'re, View = R>> TermVisitor<'re, R> for ConstByIndex {
    ...
    fn visit_const(self, symbols: &SymbolTable, cst: Name) -> Self::Value {
        if symbols.write().insert_name(cst) == self.0 {
            ()
        } else {
            todo!("error handling")
        }
...
impl<'re, R: TermReader<'re, View = R>> TermVisitor<'re, R> for ConstByGlobal {
    ...
    fn visit_const(self, symbols: &SymbolTable, cst: Name) -> Self::Value {
        ...
        let sym_idx = symbols_mut.insert_name(cst);
        let sym = &symbols_mut[sym_idx];
        if sym.parent() == SymbolIndex::PUBLIC && sym.ident() == self.0 {
            ()
        } else {
            todo!("error handling")
        }
    ...
    H: TermVisitor<'re, R>,
{
    ...
    fn visit_const(self, symbols: &SymbolTable, cst: Name) -> Self::Value {
        if symbols.write().insert_name(cst) == SymbolIndex::INTERNAL_LIST_NIL {
            None
        } else {
            todo!("error handling");
        }
